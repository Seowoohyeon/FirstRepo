...
public class S03_KakaoImageButton extends JButton {
   ...
   static {
	      try {
	         ...
	         btn_images.put(RYON, src.getSubimage(0, 0 , crop_width, crop_height));
	         btn_images.put(APEACH, src.getSubimage(crop_width / 2 ,43,crop_width, crop_height));
	         btn_images.put(MUJI, src.getSubimage(42, src.getHeight() / 2 + 8,crop_width, crop_height));
   ...
   public S03_KakaoImageButton(int picture, int x, int y, int width, int height) {
      ...
      setIcon(new ImageIcon
            (btn_images.get(picture).getScaledInstance(width, height, Image.SCALE_SMOOTH)));
