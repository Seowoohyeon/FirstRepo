...
public class D07_HashMap {
	...
	public static void main(String[] args) {
		...
		record.put("김길동", 35000);
		record.put("박길동", 90000);
		Integer prvious_value = record.put("홍길동", 3000); // 이미 존재하는 키값에 put하면 값을 수정
		
		System.out.println("이전에 들어있던 값 : " + prvious_value);
		System.out.println("지금 들어있던 값 : " + record.get("홍길동"));
	...
	//	System.out.println(person.get("싫어하는 것"));
		...
		/* 
		 	# 반복문으로 map을 활용하기
			  - ketSet() : 키들로만 이루어진 셋을 반환한다
			  - values() : value들로만 이루어진 셋을 반환한다
			  - entrySet() : Entry<key, value> 로 이루어진 셋을 반환한다
		*/
